...
static rt_uint8_t tx_msg_pool[16] = {0};
...
static void cook_com_rx_thread_entry(void *param)
{
    while (1)
    {
        ...
        {
            rt_kprintf("Cooker received: %02x %02x %02x %02x", rx_buffer[0], rx_buffer[1], rx_buffer[2], rx_buffer[3]);
        }
    }
}
...
static int cook_com_thread_init(void)
{
    ...
    cooker_drv_init(cooker_receive_irq);
...
INIT_APP_EXPORT(cook_com_thread_init);
