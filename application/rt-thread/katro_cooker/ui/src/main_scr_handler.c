...
static void main_scr_timer_handler(lv_timer_t * tmr)
{
    ...
    {
        cooker_ui_state_set(&cur_cooker_state[i], i + 1);
    }
...
void main_screen_custom_load_start(void) 
{
    ...
    for (int i = 0; i < UI_COOKER_NUM; i++)
    {
        cooker_ui_state_get(&cur_cooker_state[i], i + 1);
    }
    ...
    for (int i = 0; i < UI_COOKER_NUM; i++)
    {
        ...
        if (cur_cooker_state[i].fault || cur_cooker_state[i].gear || 
            cur_cooker_state->on_timing || cur_cooker_state->thermos)
        {
            ...
            else
            {
                ...
                // }
                // else if (cur_cooker_state->thermos)
                // {
                //     lv_obj_clear_flag(cooker_ui[i].state, LV_OBJ_FLAG_HIDDEN);
                // }
                if (cur_cooker_state[i].gear > 0)
                {
